...
describe('ICE_AGE_MISSION snow chunk lifecycle', () => {
  ...
  it('does not bank up respawn time while the field is full', () => {
    ...
    ICE_AGE_MISSION.onEnter(s);
    s.gx = -9999;
    // Force the field to cap deterministically — relying on the natural
    // spawn loop racing the assertion was flaky.
    ...
    s.missionScene.snowChunkSpawnTimer = 999;
    // One tick at cap clamps the timer.
    ICE_AGE_MISSION.update(s, 0.05);

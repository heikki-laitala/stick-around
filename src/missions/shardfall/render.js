...
export function drawShards(ctx, scene) {
  ...
  for (const s of scene.shards) {
    ...
    ctx.beginPath();
    ctx.arc(s.x, s.y, SHARD_RADIUS * pulse * 1.1, 0, Math.PI * 2);
    ctx.fill();
    // Bright core.
    ctx.shadowBlur = 0;
...
/**
 * Soft blue vignette around the screen edges while stasis is active —
 * gives the player unmistakable feedback that the world is slowed
 * without obscuring the play field.
 */
export function drawStasisVignette(ctx, W, H) {
  ctx.save();
  ...
  grad.addColorStop(1, 'rgba(80, 140, 220, 0.35)');
  ctx.fillStyle = grad;
